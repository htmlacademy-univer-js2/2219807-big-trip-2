import {getDifferenceDates} from '../utils/util';
import {BLANK_POINT, TRIP_TYPES} from '../utils/const';
...
const renderDate = (dateFrom, dateTo, isDisabled) => (
  `<div class="event__field-group  event__field-group--time">
    ...
    <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${getDifferenceDates(dateFrom)} ${isDisabled ? 'disabled' : ''}">
    &mdash;
    <label class="visually-hidden" for="event-end-time-1">To</label>
    <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${getDifferenceDates(dateTo)} ${isDisabled ? 'disabled' : ''}">
  </div>`
);

const renderType = (currentType, isDisabled) => TRIP_TYPES.map((type) => `<div class="event__type-item">
...
export default class PointEditForm extends AbstractStatefulView {
  ...
  static parsePointToState = (point, offersByType, destinations) => ({
    ...point,
    offers: offersByType.find((offer) => offer.type === point.type).offers.map((offer) => ({
      ...offer,
  ...
  #pointTypeChangeHandler = (evt) => {
    ...
    this.updateElement({
      ...
      offers: this.#offersByType.find((offer) => offer.type === type).offers.map((offer) => ({
        ...offer,
        isChecked: false
      })),
    });
  ...
  #saveClickHandler = (evt) => {
    evt.preventDefault();
    if (this._state.basePrice > 0 && getDifferenceDates(this._state.dateFrom, this._state.dateTo)) {
      this.#saveClick(PointEditForm.parseStateToPoint(this._state));
    } else {
      this.shake();
    }
  };
  ...
  #deleteClickHandler = (evt) => {
    evt.preventDefault();
    this.#deleteClick(PointEditForm.parseStateToPoint(this._state));
  };
